...
# global counters
NODE_NAME = "nsu=http://microsoft.com/Opc/OpcPlc/;s=FastUInt1"
...
class HubManager(object):
    ...
    async def prepare_infer_input(self, message):# message is OPC msg body?? or message.data is OPC msg body?
        ...
        if not message_bytes:
            return None
        #print("cc test message_bytes is ",message_bytes)  
        
        message_str = message_bytes.decode("utf-8")
        #print("cc test convert to string: ",message_str, type(message_str))  # OK
        message_obj = json.loads(message_str) # convert to a list of json objects.
        ...
        for i_msgObj in range(len(message_obj)):
            if message_obj[i_msgObj]["MessageType"] == "ua-data":             
                for i_msg in range(len(message_obj[i_msgObj]["Messages"])):   
                    ...
                    if NODE_NAME in message_obj[i_msgObj]["Messages"][i_msg]["Payload"]:
                        #print("Preparing model input {}...".format(NODE_NAME))
                        print(time.time())
                        #node_value = message_obj[i_msgObj]["Messages"][i_msg]["Payload"][NODE_NAME]["Value"]
                        #node_ts = message_obj[i_msgObj]["Messages"][i_msg]["Payload"][NODE_NAME]["SourceTimestamp"] # str type
                        #print("model input node_ts, node_value", node_ts, node_value)
                        #self.input_set.append([node_ts, node_value])
    ...
    async def message_handler(self, message):
        ...
        if message.input_name == "input1":
            try:
                ...
                await self.prepare_infer_input(message)
